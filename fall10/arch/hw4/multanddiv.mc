...
82:mar := sp; rd;			        { my attempt at MULT starts here }
83:c := 0;rd;
84:a := mbr;if n then goto 90;          
85:b := mbr; 
86:ac := band(ir, smask);
87:ac := ac + (-1): if z goto 95;            {this is where my loop begins}
88:a := a + b; if n goto 98;
89:goto loop;
90:mbr := inv(mbr);
91:mbr := mbr + 1;
92:c:= 1; goto 90;
...
95:c := c + (-1); if z then goto 93;      {loop ends here}
96:mbr:= a; wr;
97:wr :goto 0;
98:ac:= (-1);                
99:mbr:= b; wr;
...
101:mar:= sp; rd;                                
102:d:=0;rd;
103:b:= mbr; if n then goto 111;                 
104:mar:= sp + 1 ; rd; 
105:rd;
106:a:=mbr; if z then goto 133;
107:alu:= a; if n then goto 126;
108:c:=1;
109:b:=b + a; if n then goto 115;            {start of my loop}
110:c:= c + 1; goto 109 ;
111:mbr:= inv(mbr) ;         
112:mbr:= mbr+1;
113:d:= d+1; goto 103;
114:d:= d+1; goto 107;                                     
115:a:= inv(a);                                  {finished with loop}
116:a:= a + 1;
117:mbr:= b + a;
118:sp:= sp + (-1);
119:mar:= sp;wr;
120:sp:= sp + (-1);wr;
121:d:= d + (-1); if z goto 124;
122:mar:= sp; mbr:= c;wr;                       
123:ac:= 0 ;wr; goto 0;
124:c:= inv(c);                                    
125:c:= c + 1; goto 122;
