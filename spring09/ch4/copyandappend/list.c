...
extern bool set_equal(list L1, list L2, int (*p_cmp_f)()) {
  ...
  while( (temp = list_iterator(L1, temp)) != NULL ) {
    ...
    if( find_key( L2, DATA(L1), p_cmp_f, L3) == ERROR ) return FALSE ;
...
extern status copylist( list L1, list *p_L2, int size ) {
  ...
  generic_ptr *tempdata = (generic_ptr * ) malloc ( sizeof(size) ) ;

  while( (temp = list_iterator(L1, temp)) != NULL ) {
    
    *tempdata = DATA( temp ) ;
    
    append(p_L2, *tempdata) ;
...
extern status appendlist(list *p_L1, list *p_L2) {
  ...
  for (tmplist = *p_L1; NEXT(tmplist)!=NULL; tmplist=NEXT(tmplist)) ;
  NEXT(tmplist) = *p_L2;
