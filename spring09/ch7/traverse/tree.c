...
extern status bst_insert( tree *p_S, generic_ptr element, int (*p_cmp_f)() ) {

  tree T = NULL;
  int cmp;

  if ( empty_tree (*p_S) == TRUE ) {

    if ( make_root(&T, element, NULL, NULL) == ERROR) return ERROR;
    ...
    *p_S = T;
    ...
    cmp = (*p_cmp_f)(element, DATA(*p_S));
    if ( cmp < 0 )
      return bst_insert( &LEFT(*p_S), element, p_cmp_f );
    if ( cmp > 0 )
      return bst_insert( &RIGHT(*p_S), element, p_cmp_f );
    if ( cmp == 0 )
      return OK;
  }
  return OK;
