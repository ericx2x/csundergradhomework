...
Enemy::Enemy( Level *l, DrawEngine *de, tsprite s_index, 
	      float x, float y,
...
void Enemy::simulateAI( void ) {
  ...
  // you might need to multiply the magnitude by some int first,
  // depending on how fast your processor is...
  direction.x = direction.x / (mag * 10) ;
  direction.y = direction.y / (mag * 10) ;
	 
  if ( !move( direction.x, direction.y ) ) {

    // try to go in any of four possible directions ( brute force )
    // using expression rand() % 3 - 1 to create a value between -1 and 1
