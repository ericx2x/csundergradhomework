...
Fireball::Fireball( Level *lev, DrawEngine *de, tsprite s_index,
		    float x, float y, float xDir, float yDir,
...
void Fireball::idleUpdate( void ) {
  ...
  if ( Sprite::move( facingDirection.x, facingDirection.y ) ) {
    ...
    // Iterate through the list of sprites to check if we are at
    // the same x, y as one of them.
    for ( Iter = level->npc.begin() ; Iter != level->npc.end() ; Iter++ ) {
      
      // If the Sprite found is not ourself, and it shares the same
      // x and y value - then this statement is true.
