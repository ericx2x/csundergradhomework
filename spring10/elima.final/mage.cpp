...
Mage::Mage( Level *l, DrawEngine *de, tsprite s_index,
	    float x, float y, int lives, 
	    char spell_key, char up_key,
	    char down_key, char left_key, char right_key ) :
...
void Mage::castSpell( void ) {
  ...
  Fireball *temp = new Fireball( level, drawArea, SPRITE_FIREBALL, 
				 static_cast<int>(pos.x) + facingDirection.x,
				 static_cast<int>(pos.y) + facingDirection.y,
