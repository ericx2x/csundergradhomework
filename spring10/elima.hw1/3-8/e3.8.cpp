...
// This is the main program. As you can see exception handling has been taken care of and testing has occured.
  ...
  int main() {
    Company1 c1;
    try {
      c1.setName(-1, "john");  //sets name to john
    } catch(range_error& e) {
      cout << "setting name " << e.what() << endl;
    }
    try {
      c1.setName(0, "john"); //sets name to john
      c1.setSalary(0, 20); //sets salary to 20
      cout << "name should be john " << c1.name(0) <<  " salary should be 20 " << 
          c1.salary(0) << endl;
    } catch(range_error& e) { //finds error
      cout << e.what() << endl;
    }    
     try {
      Company2 c2(-1);
    ...
    try {
     Company2 c2(1);
     cout << c2.id(1) << endl; //prints id
    }  catch(range_error& e) {
      cout << "accessing 1-st employee " << e.what() << endl; 
    }     
    try {
     Company2 c2(1);
     cout << "id = " << c2.id(0) << endl; //prints id
     c2.setName(0, "Mary"); //sets name to mary
     c2.setSalary(0, 20000); //sets salary to 20000
